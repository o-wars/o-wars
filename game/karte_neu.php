...
$path = './temp/kartenew_'.$_SESSION['user']['omni'].'_'.$own_position['z'].'tmp';
...
////// activate to debug ///////////
//$time['karte'] = 0; $karte .= '<br /><b style="text-decoration:blink" class="red">DEBUG MODE ACTIVE</b>';
////////////////////////////////////
...
<table cellspacing="0" style="width:450px;" border="1" class="standard">
	...
	<tr align="center">
		<td align="center" style="width:330px;">
			Spieler: <b><font id="name">'.$row['name'].'</font></b> // Clan: <b><font id="clan">'.$row['clan'].'</font></b><br />Basis: <b><font id="base">'.$row['base'].'</font></b> // UBL: <b><font id="ubl">'.$_GET['id'].'</font></b><br />Entfernung: <b><font id="dist">'.($entfernung*2.5).'</font></b> Kilometer<br /><i>Tr&uuml;mmerfeld</i> Metall: <b><font id="tf_e">'.$row['tf_eisen'].'</font></b> Titan: <b><font id="tf_t">'.$row['tf_titan'].'</font></b>
		</td>
		<td align="center" style="width:120px;">
			<form enctype="multipart/form-data" action="?'. SID .'" method="post">
...
$result = mysql_query("SELECT clanid FROM `clans` WHERE userid < ".($map['end'])." and userid > ".($map['end']-500)." GROUP BY clanid;");

for ($row = mysql_fetch_array($result); $row ;$row = mysql_fetch_array($result)) {

	$claninfo   = mysql_query("SELECT name FROM `clan_info` WHERE clanid = ".($row['clanid'])." LIMIT 1;");
	list($name) = mysql_fetch_array($claninfo);
	$clans .= '<tr class="standard"><td align="center" onClick="clan('.$row['clanid'].', '.($map['end']/500-1).')">'.$name.'</td></tr>';
...
<table border="0" cellspacing="0">
	<tr>
		<td align="left" valign="top" style="width:140px">
			<br />
			<table border="1" cellspacing="0" class="standard" style="width:135px">
				<tr>
					<th align="center">
						Clans
					</th>
					'.str_replace('SID', 'SI<font>D</font>' ,$clans).'
				</tr>
			</table>
		</td>
		<td>
<table border="1" cellspacing="0" class="karte">
	<tr>
		<th align="center" colspan="25">
			<b>Karte Sektor '.($map['end']/500-1).':</b>
		</th>
	</tr>';
...
//	if ($tf_ges > 2500) { $img = 't.gif'; }
	...
	if ($map['start'] == $row['omni']){
		$onclick = 'onClick="t('.$map['start'].');"';
	} else {
		$onclick = '';
	}
	
	$karte .= '<td class="'.$status.'" '.$onclick.' onMouseOver="s('.$map['start'].')" onMouseOut="k('.$map['start'].')" id="'.$map['start'].'"><img id="i'.$map['start'].'" src="img/'.$img.'" /></div></td>';
...
-->
		...
		<td align="right" valign="top" style="width:140px">
			...
			<table border="1" cellspacing="0" class="standard" style="width:135px">
				...
				<tr class="standard">
					<td align="center" class="standard" onclick="status('.($map['end']/500-1).', 10);">
						Neutral
					</td>
				</tr>
				<tr class="standard">
					<td align="center" class="standard" onclick="status('.($map['end']/500-1).', 1);">
						Farmland
					</td>
				</tr>
				<tr class="standard">
					<td align="center" class="standard" onclick="status('.($map['end']/500-1).', 2);">
						Feindlich
					</td>
				</tr>
				<tr class="standard">
					<td align="center" class="standard" onclick="status('.($map['end']/500-1).', 3);">
						NAP/BND
					</td>
				</tr>
				<tr class="standard">
					<td align="center" class="standard" onclick="status('.($map['end']/500-1).', 4);">
						mein Clan
					</td>
				</tr>
				<tr class="standard">
					<td align="center" class="standard" onclick="status('.($map['end']/500-1).', 5);">
						Kriegsgegner
					</td>
				</tr>
				<tr class="standard">
					<td align="center" class="standard" onclick="status('.($map['end']/500-1).', 6);">
						Tr&uuml;mmerfelder
					</td>
				</tr>	
				<tr class="standard">
					<td align="center" class="standard" onclick="status('.($map['end']/500-1).', 7);">
						Inaktive
					</td>
				</tr>	
			</table>			
