...
if ($_SESSION['admin'] != '1'){ die('<form enctype="multipart/form-data" action="admin.php?SID" method="post">
			<b>
...
if ($_POST['ubl']){
	...
	if ($array) {
		...
		do {
			if ($array[$a[$i]]) {
				...
				$row = mysql_fetch_array($result);
				if ($_POST['ubl'] != $a[$i]){
					$multi .= '&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; <a href="admin.php?ubl='.$a[$i].'">UBL: '.$a[$i].'  - &nbsp;'.$row['name'].' ['.$array[$a[$i]].']</a><br />';
				}
			}
			$i++;
		} while ($a[$i]);
	...
	$content_right .= '<b>Ressourcen &Uuml;berweisen:</b><br />
