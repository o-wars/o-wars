...
// pages
$result = mysql_query("SELECT * FROM `user` WHERE `omni` >0 ORDER BY `points` DESC;");
...
do {
	...
	else { $pages .= ' <a href="rank.php?'.SID.'&amp;page='.$i.'">['.$i.']</a>'; }
} while ($c > $i);
$i = $_GET['page'] * 100-100;
$limit = ($_GET['page'] * 100-100).','.(100);


$select = "SELECT * FROM `user` WHERE `omni` >0 ORDER BY `points` DESC LIMIT ".$limit.";";
