...
// pages
$result = mysql_query("SELECT * FROM `user` WHERE `omni` >0 ORDER BY `gesamtpunkte` DESC;");
...
do {
	...
	else { $pages .= ' <a href="gesamtpunkte.php?'.SID.'&amp;page='.$i.'">['.$i.']</a>'; }
} while ($c > $i);
$i = $_GET['page'] * 100-100;
$limit = ($_GET['page'] * 100-100).','.(100);

$select = "SELECT * FROM `user` WHERE `omni` >0 ORDER BY `gesamtpunkte` DESC LIMIT ".$limit.";";
...
do {
	...
	if ($row){
		...
		if ($row['omni'] == $_SESSION['user']['omni']){
			...
			$newpiece = tag2value('points','<b>'.number_format($row['gesamtpunkte'],0,'','.').'</b>', $newpiece);
			$ranking .= $newpiece;
		} else {
			...
			$newpiece = tag2value('points',number_format($row['gesamtpunkte'],0,'','.'), $newpiece);
			$ranking .= $newpiece;
		}
	}
} while($row);
