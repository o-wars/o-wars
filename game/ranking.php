...
$piece = template('ranking_piece');

if ($_GET['list'] == 'ap') {
	
	$list = 'points';
	$name = 'Ausbaupunkte';
	$desc = 'Die '.$name.' werden alle 6 Stunden neu berechnet.';

} elseif ($_GET['list'] == 'kp') {
	
	$list = 'kampfpunkte';
	$name = 'Kampfpunkte';
	$desc = 'Die '.$name.' sind immer aktuell.';
...
// pages
$result = mysql_query("SELECT * FROM `user` WHERE `omni` >0 ORDER BY `".$list."` DESC;");
...
do {
	...
	else { $pages .= ' <a href="ranking.php?'.SID.'&amp;page='.$i.'&amp;list='.$_GET['list'].'">['.$i.']</a>'; }
} while ($c > $i);
$i = $_GET['page'] * 100-100;
$limit = ($_GET['page'] * 100-100).','.(100);


$select = "SELECT * FROM `user` WHERE `omni` >0 ORDER BY `".$list."` DESC LIMIT ".$limit.";";
...
do {
	...
	if ($row){
		...
		if ($row['omni'] == $_SESSION['user']['omni']){
			$newpiece = tag2value('name','<b><a href="profil.php?ubl='.$row['omni'].'&amp;'.SID.'">'.$row['name'].'</a></b>', $piece);
			...
			$newpiece = tag2value('points','<b>'.number_format($row[$list],0,'','.').'</b>', $newpiece);
			$ranking .= $newpiece;
			
		} else {
			
			$newpiece = tag2value('name','<a href="profil.php?ubl='.$row['omni'].'&amp;'.SID.'">'.$row['name'].'</a>', $piece);
			...
			$newpiece = tag2value('points',number_format($row[$list],0,'','.'), $newpiece);
			$ranking .= $newpiece;
		}
	}
	unset($img);
...
$content = tag2value('desc', $desc, $content);
