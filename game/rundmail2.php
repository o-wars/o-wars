...
if ($_SESSION[user][omni] != 1) die('nix is!');
...
if ($_POST['send'] == 1 AND $_POST['text']){
	...
	$to = mysql_query("SELECT * FROM `user`;");
	...
	for ($row=mysql_fetch_array($to);$row;$row=mysql_fetch_array($to)) {
		...
		$select = "INSERT INTO `nachrichten` ( `id` , `from` , `from_name` , `to` , `timestamp` , `gelesen` , `subject` , `text` ) VALUES ('', '".$_SESSION['user']['omni']."', '".$_SESSION[user][name]."', '".$row['omni']."', '".date(U)."', '0', '[b]Rundmail:[/b] ".substr($_POST['subject'],0,50)."', '".$_POST['text']."');";
...
// -->
</script><center><span style="font-size: 12px";><b>Rundmail schreiben:</b><form name="notes" enctype="multipart/form-data" action="rundmail2.php?'. SID .'" method="post"><table border="1" cellspacing="0" style="background-color:#e2e2e2; font-size: 12px"><tr style="background-image:url(templates/standard/table_head.gif);" align="center"><td style="width:80px">Datum</td><td style="width:180px">An:</td><td style="width:300px">Betreff</td></tr>';
$content .= '<tr align="center"><td>'.date("H:i d.m.y").'</td><td class="red">alle Spieler</td><td><input type="text" name="subject" value="'.stripslashes($_POST['subject']).'" style="width:300px" maxlength="50" /></td></tr><tr align="left"><td colspan="4"><br /><center><textarea name="text" style="width:570px; height:400px" onChange=notecheck(this) onFocus=notecheck(this) onBlur=notecheck(this) onKeyDown=notecheck(this) onKeyUp=notecheck(this)>'.html_entity_decode("\n\n\n\n".$_SESSION['user']['sig']).'</textarea><br /><input name="status" value=\'Rest: 2000\' style="width:170px;" readonly /></center></td></tr>';
